<script setup lang="ts">
    ...
    const {createTipoServicio, getTiposServicios, updateTipoServicioState, updateTipoServicio } = useTipoServicio();
    ...
    const loadRows = async (page:number=1) => {
        ...
        try {
            const data:any = await getTiposServicios({term:term.value, page, per_page:per_page.value});
    ...
    const save = async () => {
        try {
            if(form.value.id > 0)
                await updateTipoServicio(form.value.id, form.value);
            else
                await createTipoServicio(form.value);
    ...
    const updateState = async (id:number) => {
        updateTipoServicioState(id).then(() => {
...
<template>
    <div>
        <div class="font-semibold text-2xl mb-4">Tipos de servicio</div>
