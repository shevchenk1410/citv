<script setup lang="ts">
    ...
    const loadRows = async (page:number=1) => {
        ...
        try {
            const data:any = await getTiposAmbitos({term:term.value, page, per_page:per_page.value});
    ...
    const save = async () => {
        try {
            if(form.value.id > 0)
                await updateTipoAmbito(form.value.id, form.value);
            else
                await createTipoAmbito(form.value);
...
<template>
    <div>
        <div class="font-semibold text-2xl mb-4">Tipos de Ambitos</div>
